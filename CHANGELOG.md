...
- Bumped version to 4.0.1 across all manifests (package.json, tauri.conf.json, Cargo.toml, Cargo.lock) for release consistency. No user-facing changes.
...
- Fixed Track detail route (`/track/:id`) incorrectly forcing collapsed sidebar state, causing compressed UI on track pages.
- Fixed Atmos quality badge size and style for better visual balance in track listings (smaller icon + text badge).
- Improved fullscreen player UI with dedicated quality line (quality/bits/sample rate, e.g. `Dolby Atmos · 24-bit · 192 kHz`).
...
- Fixed Appwrite sync errors (unknown attribute `history_chunk_count`) when Appwrite collection schema does not support chunk fields; now uses safe truncated history fallback and retries.
- Fixed recent-history mis-logging when track play is started but not actually listened: now history entries are recorded only after 5 seconds of playback (or 10% duration) to avoid phantom recent plays.
