...
export class HiFiClient {
    ...
    static #albumTracksMax = 20;
    static #albumTracksActive = 0;
    static #albumTracksQueue = [];
    ...
    static setToken(token, expiry = -1) {
        HiFiClient.token = token;
        HiFiClient.appTokenExpiry = expiry;
    }

    static async #fetchAppToken(signal = new AbortController().signal, clientId, clientSecret) {
    ...
    static async initialize(options = {}) {
        ...
        const instance = new HiFiClient(options);
        HiFiClient.#instance = instance;
    ...
    static async #safeJson(response) {
        ...
        try {
            ...
            throw new ResponseError(response.status, `Failed to read response body: ${err.message}`);
        }
        ...
        try {
            ...
            throw new ResponseError(response.status, `Invalid JSON response from ${response.url}: ${err.message}`);
        }
    }

    async #fetchJson(url, params, signal = new AbortController().signal) {
        ...
        return HiFiClient.#safeJson(res);
    }

    static #extractUuidFromTidalUrl(href) {
        if (!href) return null;
        const parts = href.split('/');
        return parts.length >= 9 ? parts.slice(4, 9).join('-') : null;
    }

    async #withAlbumTrackSlot(fn) {
    ...
    async getTrack(id, quality = 'HI_RES_LOSSLESS', signal) {
    ...
    async getSimilarArtists(id, cursor, signal) {
    ...
    async getSimilarAlbums(id, cursor, signal) {
    ...
    async getArtist(id, f, skip_tracks = false, signal, options = {}) {
        ...
        const common_params = { countryCode: this.countryCode, limit: 50 };
        ...
        if (!skip_tracks) {
            ...
            const toptracks_params = { countryCode: this.countryCode, limit: limit || 15 };
            if (offset !== undefined) {
        ...
        const album_ids = unique_releases.map((i) => i.id).filter(Boolean);
        ...
        const fetchAlbumTracks = async (album_id) => {
            return await this.#withAlbumTrackSlot(async () => {
                ...
                const tracks = items.map((t) => (t.item ? t.item : t));
                return tracks;
            });
        };
    ...
    #buildCoverEntry(cover_slug, name, track_id) {
    ...
    async getAlbum(id, limit = 100, offset = 0, signal) {
        const albumUrl = `https://api.tidal.com/v1/albums/${id}`;
        const itemsUrl = `https://api.tidal.com/v1/albums/${id}/items`;
        const tasks = [this.#fetchJson(albumUrl, { countryCode: this.countryCode }, signal)];
    ...
    async getMix(id, signal) {
        ...
        return { version: API_VERSION, mix: header, items: items.map((it) => (it.item ? it.item : it)) };
    }

    async getPlaylist(id, limit = 100, offset = 0, signal) {
    ...
    async getLyrics(id, signal) {
        ...
        if (!data) {
            const err = new Error('Lyrics not found');
            err.status = 404;
            throw err;
        }
    ...
    async getVideo(id, quality = 'HIGH', mode = 'STREAM', presentation = 'FULL', signal) {
    ...
    async queryResponse(pathOrUrl, signal) {
        try {
