export class MusicDatabase {
    ...
    async open() {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                // New Recently Played and Recommendation Engine Stores
                if (!db.objectStoreNames.contains('recent_activity')) {
                    const store = db.createObjectStore('recent_activity', { keyPath: 'pk' });
                    store.createIndex('lastPlayed', 'lastPlayed', { unique: false });
                    store.createIndex('type', 'type', { unique: false });
                }
                if (!db.objectStoreNames.contains('scrobbles')) {
                    const store = db.createObjectStore('scrobbles', { keyPath: 'timestamp' });
                    store.createIndex('trackId', 'trackId', { unique: false });
                }
                if (!db.objectStoreNames.contains('user_track_stats')) {
                    db.createObjectStore('user_track_stats', { keyPath: 'id' });
                }
                if (!db.objectStoreNames.contains('user_artist_stats')) {
                    db.createObjectStore('user_artist_stats', { keyPath: 'id' });
                }
                if (!db.objectStoreNames.contains('taste_profile')) {
                    db.createObjectStore('taste_profile', { keyPath: 'id' });
                }
                if (!db.objectStoreNames.contains('recommendation_cache')) {
                    db.createObjectStore('recommendation_cache', { keyPath: 'id' });
                }
            };
        });
    }
    ...
    _getFavoriteCandidateKeys(type, rawKey) {
        if (rawKey === null || rawKey === undefined || rawKey === '') return [];

        const keys = new Set();
        keys.add(rawKey);

        const asString = String(rawKey);
        if (asString) keys.add(asString);

        if (type !== 'playlist') {
            const asNumber = Number(asString);
            if (Number.isFinite(asNumber)) keys.add(asNumber);
        }

        return Array.from(keys);
    }

    _resolveFavoriteRawKey(type, itemOrId) {
        if (itemOrId && typeof itemOrId === 'object') {
            if (type === 'playlist') return itemOrId.uuid || itemOrId.id;
            if (type === 'track') return itemOrId.id || itemOrId.trackId || itemOrId.uuid || itemOrId.isrc;
            if (type === 'album') return itemOrId.id || itemOrId.albumId || itemOrId.uuid;
            if (type === 'artist') return itemOrId.id || itemOrId.artistId || itemOrId.uuid;
            if (type === 'mix') return itemOrId.id || itemOrId.mixId || itemOrId.uuid;
            return itemOrId.id || itemOrId.uuid;
        }
    ...
    async toggleFavorite(type, item) {
        ...
        const rawKey = this._resolveFavoriteRawKey(type, item);
        if (rawKey === null || rawKey === undefined || rawKey === '') {
            console.warn(`[DB] toggleFavorite skipped for ${type}: missing stable key`, item);
            return false;
        }
        const candidateKeys = this._getFavoriteCandidateKeys(type, rawKey);
        const key =
            candidateKeys.find((candidate) => typeof candidate === 'string' && candidate !== '') ||
            candidateKeys[0] ||
            rawKey;
        const exists = await this.isFavorite(type, rawKey);

        if (exists) {
            await this.performTransaction(storeName, 'readwrite', (store) => {
                candidateKeys.forEach((candidateKey) => store.delete(candidateKey));
            });
            // Sync to cloud immediately
    ...
    async isFavorite(type, id) {
        const plural = type === 'mix' ? 'mixes' : `${type}s`;
        const storeName = `favorites_${plural}`;
        const rawKey = this._resolveFavoriteRawKey(type, id);
        const candidateKeys = this._getFavoriteCandidateKeys(type, rawKey);

        for (const key of candidateKeys) {
            try {
                const result = await this.performTransaction(storeName, 'readonly', (store) => store.get(key));
                if (result) return true;
            } catch {
                // Continue checking alternate key representations.
            }
        }
    ...
    // Recently Played API (Enhanced)
    async addRecentActivity(type, item) {
        const storeName = 'recent_activity';
        const minified = this._minifyItem(type, item);
        if (!minified) return;

        const id = type === 'playlist' ? minified.uuid : minified.id;
        const pk = `${type}:${id}`;
        const entry = {
            ...minified,
            pk,
        ...
        if (typeof window !== 'undefined') {
            window.dispatchEvent(new CustomEvent('recent-activity-changed', { detail: { type, item: entry } }));
        }

        return entry;
    }

    async getRecentActivity(type = null, limit = 50) {
        const storeName = 'recent_activity';
        const db = await this.open();
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                let results = request.result.reverse(); // Newest first
                if (type) {
    ...
    async clearRecentActivity(type = null) {
        const storeName = 'recent_activity';
        const db = await this.open();

        if (!type) {
            return await this.performTransaction(storeName, 'readwrite', (store) => store.clear());
        }

        // If type specified, we need to delete matching items one by one or via cursor
        return new Promise((resolve, reject) => {
            const transaction = db.transaction(storeName, 'readwrite');
            const store = transaction.objectStore(storeName);
    ...
    async saveScrobble(scrobble) {
        const storeName = 'scrobbles';
        const entry = {
            ...scrobble,
            timestamp: scrobble.timestamp || Date.now(),
        };
        await this.performTransaction(storeName, 'readwrite', (store) => store.put(entry));
        return entry;
    }

    async getScrobbles(limit = 1000) {
        const storeName = 'scrobbles';
        const db = await this.open();
        ...
        return new Promise((resolve, reject) => {
            const transaction = db.transaction(storeName, 'readonly');
            const store = transaction.objectStore(storeName);
            const request = store.getAll(null, limit); // Simplified limit handling
    ...
    async saveUserTrackStats(id, stats) {
        await this.performTransaction('user_track_stats', 'readwrite', (store) => store.put({ id, ...stats }));
    }

    async getUserTrackStats(id) {
        return (await this.performTransaction('user_track_stats', 'readonly', (store) => store.get(id))) || null;
    }

    async saveUserArtistStats(id, stats) {
        await this.performTransaction('user_artist_stats', 'readwrite', (store) => store.put({ id, ...stats }));
    }

    async getUserArtistStats(id) {
        return (await this.performTransaction('user_artist_stats', 'readonly', (store) => store.get(id))) || null;
    }

    async saveTasteProfile(id, profile) {
        await this.performTransaction('taste_profile', 'readwrite', (store) => store.put({ id, ...profile }));
    }

    async getTasteProfile(id = 'default') {
        return (await this.performTransaction('taste_profile', 'readonly', (store) => store.get(id))) || null;
    }

    async saveRecommendationCache(id, data) {
        await this.performTransaction('recommendation_cache', 'readwrite', (store) =>
            store.put({ id, data, timestamp: Date.now() })
        );
    }

    async getRecommendationCache(id) {
        return (await this.performTransaction('recommendation_cache', 'readonly', (store) => store.get(id))) || null;
    }
    ...
    async _refreshCollaborativePlaylistsFromCloud() {
        try {
            const [{ syncManager }, { authManager }] = await Promise.all([
                import('./accounts/appwrite-sync.js'),
                import('./accounts/auth.js'),
            ]);
            if (authManager?.user && syncManager?.syncCloudCollaborativePlaylistsToLocal) {
                await syncManager.syncCloudCollaborativePlaylistsToLocal();
            }
        } catch (_error) {
            // Best-effort cloud refresh; continue with local data if unavailable.
        }
    }
    ...
    async getCollaborativePlaylists() {
        await this._refreshCollaborativePlaylistsFromCloud();
        const db = await this.open();
        return new Promise((resolve, reject) => {
