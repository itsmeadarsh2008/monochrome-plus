...
function isCorsHostLikelyBlocked(url) {
    try {
        const parsed = new URL(url, window.location.href);
        return parsed.hostname.toLowerCase() === 'resources.tidal.com';
    } catch {
        return false;
    }
}
...
export async function applyPaletteFromImage(imageUrl) {
    ...
    // On official web domain, TIDAL CDN usually blocks CORS for canvas extraction.
    // Skip extraction silently to avoid repeated CORS console noise.
    if (!isLocalBrowserDev() && isCorsHostLikelyBlocked(imageUrl)) {
        return;
    }
    ...
    try {
        ...
        if (!isCorsHostLikelyBlocked(imageUrl)) {
            // Non-fatal: album art might not load due to transient image failures.
