...
export function initializeSettings(scrobbler, player, api, ui, discord) {
    ...
    const authEmailInput = document.getElementById('auth-email-password-email');
    const authEmailValidationStatus = document.getElementById('auth-email-validation-status');
    const accountCommunityPanel = document.getElementById('account-community-panel');
    ...
    const renderCommunityStats = async () => {
        if (!accountCommunityPanel || !statTotalUsers || !statOnlineUsers || !authManager.user) return;
        accountCommunityPanel.style.display = 'grid';
    ...
    const withUiError = (error) => {
        ...
        } else if (code === 'auth/disposable-email') {
            message = 'Disposable email domains are blocked. Please use your real email address.';
    ...
    const renderAuthMode = () => {
        ...
        if (authEmailValidationStatus) {
            if (isSignup) {
                authEmailValidationStatus.style.display = 'block';
                const emailValue = authEmailInput?.value?.trim() || '';
                if (!emailValue) {
                    authEmailValidationStatus.textContent = 'Enter an email to verify if it can be used.';
                    authEmailValidationStatus.dataset.state = 'idle';
                    authEmailValidationStatus.style.color = 'var(--muted-foreground)';
                }
            } else {
                authEmailValidationStatus.style.display = 'none';
                authEmailValidationStatus.textContent = '';
                authEmailValidationStatus.dataset.state = 'idle';
                authEmailValidationStatus.style.color = 'var(--muted-foreground)';
            }
        }
    ...
    const updateSignupEmailDetector = () => {
        if (!authEmailValidationStatus || authMode !== 'signup') return;
        ...
        const result = authManager.evaluateSignupEmail(email);
        authEmailValidationStatus.textContent = result.message;
        authEmailValidationStatus.dataset.state = result.ok ? 'valid' : 'invalid';
        authEmailValidationStatus.style.color = result.ok ? '#22c55e' : '#f87171';
    };

    authEmailInput?.addEventListener('input', updateSignupEmailDetector);
    ...
    authEmailForm?.addEventListener('submit', async (event) => {
        ...
        try {
            if (authMode === 'signup') {
                const precheck = authManager.evaluateSignupEmail(email);
                if (!precheck.ok) {
                    if (authEmailValidationStatus) {
                        authEmailValidationStatus.textContent = precheck.message;
                        authEmailValidationStatus.dataset.state = 'invalid';
                    }
                    const syntheticError = new Error(precheck.message);
                    syntheticError.code = precheck.code;
    ...
    renderCommunityStats().catch((error) => {
        console.warn('[Account Community] Initial render failed:', error);
    });

    authManager.onAuthStateChanged(() => {
        renderCommunityStats().catch((error) => {
            console.warn('[Account Community] Auth-state render failed:', error);
        });
    });
    ...
    const lyricsPanelOpacitySlider = document.getElementById('lyrics-panel-opacity-slider');
    const lyricsPanelOpacityValue = document.getElementById('lyrics-panel-opacity-value');

    const applyLyricsPanelOpacity = (value) => {
        const normalized = lyricsPanelSettings.setOpacity(value);
        if (lyricsPanelOpacitySlider) lyricsPanelOpacitySlider.value = String(normalized);
        if (lyricsPanelOpacityValue) lyricsPanelOpacityValue.textContent = `${normalized}%`;
        document.documentElement.style.setProperty('--lyrics-panel-opacity', String(normalized / 100));
    };

    if (lyricsPanelOpacitySlider) {
        const savedOpacity = lyricsPanelSettings.getOpacity();
        applyLyricsPanelOpacity(savedOpacity);

        lyricsPanelOpacitySlider.addEventListener('input', () => {
            applyLyricsPanelOpacity(lyricsPanelOpacitySlider.value);
        });
    }
