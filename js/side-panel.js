export class SidePanelManager {
    constructor() {
        this.DESKTOP_SPLIT_QUERY = '(min-width: 1201px) and (hover: hover) and (pointer: fine)';
        this.MAIN_SIDE_MOBILE_BREAKPOINT = 980;
        ...
        window.addEventListener('resize', () => this._updateLayoutWidthVar());

        this._updateBodyPanelState();
    }

    _isDesktopLayout() {
        return window.matchMedia(this.DESKTOP_SPLIT_QUERY).matches;
    }

    _getPanelWidth() {
        const currentWidth = parseInt(getComputedStyle(this.panel).getPropertyValue('--side-panel-width'), 10);
        if (!isNaN(currentWidth) && currentWidth > 0) return currentWidth;
        return 600;
    }

    _updateLayoutWidthVar() {
        const isPanelOpen = this.panel.classList.contains('active');
        const panelWidth = this._getPanelWidth();
        const activeWidth = this._isDesktopLayout() && isPanelOpen ? panelWidth : 0;
        const remainingMainWidth = window.innerWidth - activeWidth;
        const useMainSideMobileMode = activeWidth > 0 && remainingMainWidth <= this.MAIN_SIDE_MOBILE_BREAKPOINT;

        document.documentElement.style.setProperty('--active-side-panel-width', `${activeWidth}px`);
        document.body.classList.toggle('side-panel-main-mobile', useMainSideMobileMode);
    }
