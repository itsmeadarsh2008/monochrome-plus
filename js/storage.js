...
export const proxySettings = {
    STORAGE_KEY: 'monochrome-proxy-settings',

    _getAll() {
        try {
            const data = localStorage.getItem(this.STORAGE_KEY);
            return data ? JSON.parse(data) : { proxies: [], enabled: false };
        } catch {
            return { proxies: [], enabled: false };
        }
    },
    ...
    getProxies() {
        const data = this._getAll();
        // Sort by latency (fastest first), proxies without latency go to the end
        return [...data.proxies].sort((a, b) => {
    ...
    addProxy(url) {
        ...
        if (data.proxies.some((p) => p.url === normalized)) return false;
        data.proxies.push({ url: normalized, latency: null });
        this._save(data);
        return true;
    },
    ...
    removeProxy(url) {
        const data = this._getAll();
        data.proxies = data.proxies.filter((p) => p.url !== url);
        this._save(data);
    },

    updateLatency(url, latency) {
        const data = this._getAll();
        const proxy = data.proxies.find((p) => p.url === url);
        if (proxy) {
    ...
    async testProxy(proxyUrl) {
        const testTarget = 'https://eu-central.monochrome.tf/';
        const url = `${proxyUrl}/${encodeURIComponent(testTarget)}`;
    ...
    async testAllProxies() {
        ...
        const results = await Promise.allSettled(
            data.proxies.map(async (p) => {
                const latency = await this.testProxy(p.url);
                p.latency = latency;
                return { url: p.url, latency };
            })
        );
        this._save(data);
    ...
    getFastestProxy() {
        ...
        return proxies.find((p) => p.latency != null) || proxies[0] || null;
    },
        ...
        try {
            ...
            if (legacyData && (!recents || recents.length === 0)) {
                try {
                    ...
                    for (const [t, items] of Object.entries(parsed)) {
                        const singularType = t.replace(/s$/, '').replace(/mixe$/, 'mix');
                        if (Array.isArray(items)) {
                            for (const item of items) {
                                await db.addRecentActivity(singularType, item);
                            }
                        }
                    }
            ...
            const grouped = { artists: [], albums: [], playlists: [], mixes: [], tracks: [] };
            if (recents) {
                recents.forEach((r) => {
                    const plural = r.type === 'mix' ? 'mixes' : `${r.type}s`;
                    if (grouped[plural]) {
                        grouped[plural].push(r);
                    }
                });
            }
            return grouped;
            ...
            return type ? [] : { artists: [], albums: [], playlists: [], mixes: [], tracks: [] };
        }
    },

    async clear(type = null) {
        await db.clearRecentActivity(type);
    },

    async addArtist(artist) {
        return await db.addRecentActivity('artist', artist);
    },

    async addAlbum(album) {
        return await db.addRecentActivity('album', album);
    },

    async addPlaylist(playlist) {
        return await db.addRecentActivity('playlist', playlist);
    },

    async addMix(mix) {
        return await db.addRecentActivity('mix', mix);
    },

    async addTrack(track) {
        return await db.addRecentActivity('track', track);
    },
};
...
export const lyricsPanelSettings = {
    ...
    getOpacity() {
        try {
            const raw = localStorage.getItem(this.OPACITY_KEY) || '';
            const parsed = Number.parseFloat(raw);
            if (!Number.isFinite(parsed)) return this.DEFAULT_OPACITY;

            // Legacy values might be stored as fractions like 0.6.
            const asPercent = parsed >= 0 && parsed <= 1 ? parsed * 100 : parsed;
            return Math.max(this.MIN_OPACITY, Math.min(this.MAX_OPACITY, Math.round(asPercent)));
        } catch {
    ...
    setOpacity(value) {
        ...
        const asPercent = Number.isFinite(parsed) && parsed >= 0 && parsed <= 1 ? parsed * 100 : parsed;
        const normalized = Number.isFinite(asPercent)
            ? Math.max(this.MIN_OPACITY, Math.min(this.MAX_OPACITY, Math.round(asPercent)))
            : this.DEFAULT_OPACITY;
        localStorage.setItem(this.OPACITY_KEY, String(normalized));
        return normalized;
    },
};
