...
function decodeCorsProxyUrl(url) {
...
async function fetchWithFallback(urls, { responseType = 'json', timeoutMs = 9000 } = {}) {
    ...
    for (const rawUrl of urls) {
        ...
        if (!raw) continue;
        const primaryUrl = /^https?:\/\//i.test(raw) ? buildCorsSafeUrl(raw) : raw;
        const requestUrls = [primaryUrl];

        const decodedFromCorsProxy = decodeCorsProxyUrl(primaryUrl);
        if (decodedFromCorsProxy && /^https?:\/\//i.test(decodedFromCorsProxy)) {
            requestUrls.push(buildAllOriginsRawUrl(decodedFromCorsProxy));
        } else if (/^https?:\/\//i.test(raw)) {
            requestUrls.push(buildAllOriginsRawUrl(raw));
        }
        ...
        for (const requestUrl of requestUrls) {
            ...
            try {
                ...
                if (!response.ok) {
                    lastError = new Error(`HTTP ${response.status} for ${requestUrl}`);
                    continue;
                }
...
function getSheetId(url) {
    ...
    // Standard Google Sheets URL: /spreadsheets/d/{sheetId}
    const directMatch = normalizedUrl.match(/\/spreadsheets\/d\/([a-zA-Z0-9-_]+)/i);
    if (directMatch?.[1] && directMatch[1].toLowerCase() !== 'e') {
        return directMatch[1];
    }

    // Published/embed URL variant: /spreadsheets/d/e/{publishedId}
    const publishedMatch = normalizedUrl.match(/\/spreadsheets\/d\/e\/([a-zA-Z0-9-_]+)/i);
    if (publishedMatch?.[1]) {
        return publishedMatch[1];
    }

    // Fallback for URL patterns that provide ?id=
    try {
...
async function fetchTrackerData(sheetId) {
    try {
        ...
        if (!safeSheetId) return null;

        const urls = TRACKER_ENDPOINTS.trackerGet.map((base) => `${base}/${safeSheetId}`);
...
function parseTrackerTrackId(trackId) {
    const prefix = 'tracker-';
    if (!trackId || !String(trackId).startsWith(prefix)) {
        return null;
    }

    const payload = String(trackId).slice(prefix.length);
    const lastDash = payload.lastIndexOf('-');
    if (lastDash <= 0) {
        return null;
    }

    const indexRaw = payload.slice(lastDash + 1);
    const trackIndex = Number.parseInt(indexRaw, 10);
    if (!Number.isFinite(trackIndex)) {
        return null;
    }

    const sheetAndEra = payload.slice(0, lastDash);

    // Prefer a known sheetId match to handle sheet IDs and era names containing dashes.
    let matchedSheetId = null;
    for (const candidateSheetId of artistBySheetId.keys()) {
        if (sheetAndEra.startsWith(`${candidateSheetId}-`)) {
            if (!matchedSheetId || candidateSheetId.length > matchedSheetId.length) {
                matchedSheetId = candidateSheetId;
            }
        }
    }

    if (matchedSheetId) {
        const encodedEraName = sheetAndEra.slice(matchedSheetId.length + 1);
        return {
            sheetId: matchedSheetId,
            eraName: decodeURIComponent(encodedEraName),
            trackIndex,
        };
    }

    // Fallback for legacy IDs where sheetId does not include dashes.
    const firstDash = sheetAndEra.indexOf('-');
    if (firstDash <= 0) {
        return null;
    }

    return {
        sheetId: sheetAndEra.slice(0, firstDash),
        eraName: decodeURIComponent(sheetAndEra.slice(firstDash + 1)),
        trackIndex,
    };
}
...
export async function renderTrackerTrackPage(trackId, container, _ui) {
    ...
    const parsedTrackId = parseTrackerTrackId(trackId);
    if (!parsedTrackId) {
        container.innerHTML = '<p style="text-align: center; padding: 2rem;">Invalid track ID.</p>';
        return;
    }

    const { sheetId, eraName, trackIndex } = parsedTrackId;
